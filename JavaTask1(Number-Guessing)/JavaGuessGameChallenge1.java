...
public class JavaGuessGameChallenge1 {
    ...
    public static void main(String[] args) {
        ...
        while (playAgain) {
            noOfRounds++;
            int randomNumber = random.nextInt(maxNum - minNum + 1) + minNum;// Generate a random number between
                                                                            // (50-100).
            int attempts = 0;
            boolean correctlyGuessed = false;

            System.out.println("\nRound " + noOfRounds + ": You have " + maximunAttempts
                    + " attempts to guess the random number.");
            ...
            while (attempts < maximunAttempts && !correctlyGuessed) {
                ...
                if (userGuess < minNum || userGuess > maxNum) { // If userguess is higher or lower then the given number
                                                                // range.
                    System.out.println(
                            "Invalid input! Please enter a valid number between " + minNum + " and " + maxNum + ".");
                    ...
                    if (!correctlyGuessed && attempts == maximunAttempts) {
                        System.out.println("🙂Sorry you've used all of your attempts. The correct number was "
                                + randomNumber + ".");
                    } else if (!correctlyGuessed) {//checking attempts and provide feedback!
